...
def binary_code(s):
    ...
    for byte in byte_arr:
        bi_code = bin(byte)
        byte_list.append(binary_convertor(bi_code))
    ...
    while i!=f:
        while(i!=(f-j)):
            ...
            if(b!="ACK lost"):
                ...
                print("[Server]Now sending the next packet")
                i+=1
                k+=1
                time.sleep(1)
            else:
                time.sleep(3)
                print("[Server]ACK of the data lost!\nSliding window range : "+str(i+1)+" to "+str(k+1))
                print("[Server]Now resending the same packet")
                time.sleep(1)
